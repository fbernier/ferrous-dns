...
/// Weekday index (0 = Mon .. 6 = Sun) as the bitmask `evaluate_slots` expects.
fn day(index: u8) -> u8 {
    1u8 << index
}
...
fn test_evaluate_slots_day_not_matching_returns_none() {
    ...
    assert_eq!(evaluate_slots(&[monday_only], day(1), "10:00"), None);
}
...
fn test_evaluate_slots_allow_only_no_conflicts_returns_allow_all() {
    ...
    assert_eq!(
        evaluate_slots(&[weekdays, weekend], day(5), "15:00"),
        Some(ScheduleAction::AllowAll)
    );
}
...
fn test_evaluate_slots_multiple_days_matches_correct_day() {
    ...
    let slots = [weekdays];
    assert_eq!(
        evaluate_slots(&slots, day(1), "12:00"),
        Some(ScheduleAction::BlockAll)
    );
    assert_eq!(evaluate_slots(&slots, day(5), "12:00"), None);
}
