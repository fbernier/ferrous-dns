use ferrous_dns_infrastructure::dns::tunneling::client_stats::{
    subnet_key_from_ip, ClientApexStats, TrackingKey,
};
...
fn ipv4_same_subnet_produces_same_key() {
    ...
    assert_eq!(subnet_key_from_ip(a), subnet_key_from_ip(b));
}
...
fn ipv4_different_subnet_produces_different_key() {
    ...
    assert_ne!(subnet_key_from_ip(a), subnet_key_from_ip(b));
}
...
#[test]
fn ipv4_mapped_ipv6_client_keys_like_its_ipv4_address() {
    let mapped: IpAddr = "::ffff:192.168.1.5".parse().unwrap();
    let native: IpAddr = "192.168.1.5".parse().unwrap();
    let other_mapped: IpAddr = "::ffff:10.0.0.1".parse().unwrap();
    assert_eq!(subnet_key_from_ip(mapped), subnet_key_from_ip(native));
    assert_ne!(subnet_key_from_ip(mapped), subnet_key_from_ip(other_mapped));
}
